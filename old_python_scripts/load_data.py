...
if not os.path.exists(all_data_file_path):
    ...
    for folder in os.listdir(parent_dir):
        ...
        if os.path.isdir(folder_path):
            ...
            if len(tissue_organ_origin) == 0 or (tissue_organ_origin == 'unknown').any():
                continue
            else:
...
# create a boolean mask for labels that are not 'breast', 'lung', or 'prostate'
mask = (all_labels[0].isin(['breast', 'lung', 'prostate']))

# use the mask to index the all_labels dataframe and print the resulting subset
print(all_labels[mask])


# use the mask to index the all_labels and all_data dataframes and print the resulting subset
all_labels = all_labels.loc[mask]
...
with open(all_data_columns_file_path, 'wb') as all_data_columns_pckl: 
    pickle.dump(all_data_columns, all_data_columns_pckl)