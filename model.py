...
class Model(tf.keras.Model):
    ...
    def __init__(
        ...
        if not self.checkActivationFunction(activation_pred):
            raise RuntimeError(">> Unrecognized activation function. Acceptable options are \"relu\", \"tanh\", \"sigmoid\" and \"linear\"")
        ...
        with tf.device('/GPU:0'):
            ...
            for epoch in range(epochs):
                ...
                if (epoch + 1) % self.display_step == 0:
                    print(f">> [LSPIN] Epoch: {epoch + 1}| Train [loss: {train_loss:.4f}, acc: {train_acc * 100:.2f}%] - Test [loss: {valid_loss:.4f}, acc: {valid_acc * 100:.2f}%]")
